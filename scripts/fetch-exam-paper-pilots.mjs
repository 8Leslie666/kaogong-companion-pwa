...
const selectedIds = process.argv.slice(2).filter((value) => !value.startsWith('--'))
const paperIds = selectedIds.length ? selectedIds : defaultIds
...
console.log(JSON.stringify({ output: generatedOutput, reports: results.map((result) => result.report) }, null, 2))
