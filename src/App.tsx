...
function PracticePage({ onStart }: { onStart: (options: CreateSessionOptions) => Promise<void> }) {
  ...
  return <>
    ...
    {catalog && <details className="catalog-card"><summary><span><strong>{catalog.stats.total} 套国考与省考目录</strong><small>国考 {catalog.stats.national} 套 · 省考 {catalog.stats.provincial} 套 · 已离线接入 2 套</small></span><ChevronRight /></summary><div className="catalog-browser"><label><span>搜索年份或标题</span><input value={catalogQuery} onChange={(event) => setCatalogQuery(event.target.value)} placeholder="例如：2025、浙江、行政执法" /></label><div className="filter-chips"><button className={clsx(catalogRegion === '全部地区' && 'selected')} onClick={() => setCatalogRegion('全部地区')}>全部地区</button>{catalog.regions.map((item) => <button key={item} className={clsx(catalogRegion === item && 'selected')} onClick={() => setCatalogRegion(item)}>{item} · {catalog.stats.byRegion[item] ?? 0}</button>)}</div><div className="catalog-list">{catalogPapers.map((paper) => <a key={paper.id} href={paper.url} target="_blank" rel="noreferrer"><span><strong>{paper.title}</strong><small>{paper.region} · {paper.year ?? '年份待识别'} · {paper.variant}{paper.recallEdition ? ' · 回忆版' : ''}</small></span><ExternalLink /></a>)}</div>{catalogPapers.length === 20 && <p className="catalog-hint">当前显示前 20 套，可继续输入年份或关键词缩小范围。</p>}</div></details>}
